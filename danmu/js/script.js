...
$(document).ready(function () {
  ...
  (function ($) {
    ...
    DanmuWall.prototype.remove = function (danmu) {
      this._recycleDanmu(danmu);
      this.queue = this.queue.filter((item) => item !== danmu);
    };

    DanmuWall.prototype._recycleDanmu = function (danmu) {
    ...
    DanmuWall.prototype.getDanmu = function (message, options) {
      ...
      if (this.pool.length > 0) {
        console.log(this.pool, "before");
        danmu = this.pool.shift();
        console.log(this.pool, "after");
        // re-config the danmu instance
    ...
    $.fn.danmuwall = function () {
      if (!this.__DMW) {
        this.__DMW = new DanmuWall(this);
      }
      return this.__DMW;
    };
  })(jQuery);
  ...
  const danmuWall = $(".viewport").danmuwall();
